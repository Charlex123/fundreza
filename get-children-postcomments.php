...
if(isset($_POST['new_reply']) && $_POST['new_reply'] != "" && $_POST['new_reply'] != null && isset($_POST['code']) && $_POST['code'] != "" && $_POST['code'] != null) {
    ...
    if(!isset($user_data) && !isset($name) && !isset($userid)) {
        ...
        $fetch_c_reply = $con -> prepare("SELECT updatechildrencomments.name,updatechildrencomments.comment,users.profile_pics,updatechildrencomments.comment_time,updatechildrencomments.postcode,updatechildrencomments.commentCode,updatechildrencomments.replyCode FROM updatechildrencomments INNER JOIN users ON updatechildrencomments.name = users.name WHERE updatechildrencomments.commentCode = ? AND updatechildrencomments.name = ? ORDER BY updatechildrencomments.comment_time DESC");
