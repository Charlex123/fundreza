...
<div class="wrapper" id="main">
  ...
  <nav class="navbar navbar-expand" style="width: 100%;">
    ...
    <ul class="nav-menu list-unstyled ml-auto d-flex flex-md-row align-items-md-center">
      <li class="nav-item d-none d-sm-inline-block">
        <a rel='nofollow' style='cursor:pointer' data-toggle='modal' data-target='#editprofile' name='' aria-haspopup='true' aria-expanded='false' class="nav-link"> <i class="fas fa-user-edit"></i> Profile </a>
      </li>  
      <!-- Languages dropdown    -->
    ...
    <div class="wrapper-c">
      ...
      <div class="mfr">
            <?php require_once'../dbh.php';require_once'../config.php';
                ...
                $chk = $con -> prepare("SELECT * FROM users WHERE email = ? ");
                $chk ->bindParam(1,$email_of_user);
                $chk -> execute();

                if($chk -> execute() && $chk -> rowCount() > 0) {

                  $ud = $chk -> fetch(PDO::FETCH_ASSOC);

                  $ln = ucwords($ud['Lname']);
                  $fn = ucwords($ud['Fname']);
                  $emai = ucfirst($ud['email']);
                  $ph = @$ud['phonenumber'];
                    
                  $at = $ud['created_at'];
                  $country = $ud['userCountry'];
                  $tim = date('l jS \of F Y h:i:s A', $at);
                if($ph == null || $ph == "") {
                  $phn = 'Not provided';
                }else {
                  $phn = $ph;
            ...
            ?>
              ...
              <div class="w-r">
                <div class="cet"><h4> <i class="fas fa-user"></i> Your account details</h4></div>
                <ul>
                  <li>
                      <i class="fas fa-user icc"></i> Full name: <?php echo @$fn . ' ' . @$ln;?>
                  </li>
                  <li>
                      <i class="fas fa-envelope icc"></i> Email: <?php echo @$emai;?>
                  </li>
                  <li>
                      <?php echo "<i class='fas fa-phone icc'></i> Phone: $phn";?>
                  </li>
                  <li>
                      <i class="fas fa-flag icc"></i> Country: <?php echo @$country;?>
                  </li>
                  <li>
                      <i class="fas fa-clock icc"></i> Account created at: <?php echo @$tim;?>
                  </li>
                </ul>
              
                <div class="sml">
                  <div class="gbk">
                    <a rel='nofollow' style='cursor:pointer' data-toggle='modal' data-target='#editprofile' name='' aria-haspopup='true' aria-expanded='false' class="nf text-white w100"> <i class="fas fa-user-edit"></i> Update Profile </a>
                  </div>
                  <div class="gbk">
        ...
        <div class="modal" id="editprofile">
            ...
            <div class="modal-content">
              ...
              <form action="" method="POST" class="form-validate text-center">
                  ...
                  <div class="form-group">
                      <label for="" class="text-left"> Last name</label>
                  ...
                  <div class="form-group">
                      ...
                      <input id="phone" type="text" name="phonenumber" required placeholder="Update mobile" value="<?php echo @$_POST['phonenumber']?>" class="form-control">
                  </div>

                  <div class="form-group mx-auto text-center">
                    <button id="register" type="submit" name="editProfile" class="nf font-weight-normal mx-auto border-0 pl-3 pr-3" id='submit_account'> Update Profile</button>
                  </div>
              </form>
            </div>
        </div>
      ...
      <?php 
        ...
        if(isset($_POST['editProfile'])) {
          ...
          if(isset($fname) && htmlspecialchars($lname) && htmlspecialchars($phone)) {
              if($phone != strip_tags($_POST['phonenumber'])) {
                  $errorMsg = "invalid phonenumber";
                  
              }else if(!preg_match("/^[ 0-9+]*$/",$phone)) {
                  $errorMsg = '<div class="w100 bhe">only numbers required! <span class="close-err">&times;</span> </div>';
                  header("location:profile-deatils.php");
                  }
                  $phone;
                  $con = new PDO("mysql:host=$serverhost;dbname=fundgcmf_db;" , $serverusername, $serverpassword);
                  $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                  $upLogin = $con -> prepare("UPDATE users SET Fname = ?, Lname = ?, phonenumber = ? WHERE email = ?");
                  $upLogin -> bindParam(1,$fname,PDO::PARAM_STR);
                  $upLogin -> bindParam(2,$lname);
                  $upLogin -> bindParam(3,$phone);
                  $upLogin -> bindParam(4,$email_of_user);
                  $upLogin -> execute();
                  if($upLogin -> execute()) {
                    $errorMsg ='<div class="w100 bhe">Profile update successful <span class="close-err">&times;</span></div>';
                  }
              ...
              if(preg_match("/^[a-z A-Z0-9]*$/",$lname)) {
  
                  $phone;
                  $upLogin = $con -> prepare("UPDATE users SET Fname, Lname = ? WHERE email = ? ");
                  $upLogin -> bindParam(1,$acname,PDO::PARAM_STR);
                  $upLogin -> bindParam(2,$usertype);
                  $upLogin -> bindParam(3,$email_of_user);
                  $upLogin -> execute();
                  if($upLogin -> execute()) {
                    $errorMsg ='<div class="w100 bhe">Profile update successful <span class="close-err">&times;</span></div>';
                  }
                  header("location:profile-details.php?name=$uname");
