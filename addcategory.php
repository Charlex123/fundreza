...
<div class="wrapper" id="main">
  ...
  <nav class="navbar navbar-expand" style="width: 100%;">
    ...
    <ul class="nav-menu list-unstyled ml-auto d-flex flex-md-row align-items-md-center">
      ...
      <!-- Languages dropdown    -->
      <li class="nav-item dropdown"><a id="settings" rel="nofollow" data-target="#" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link language"><span class=''></span><i class='fas fa-angle-down'></i></a> 
      ...
      <div class="admin-wrapper">
            <?php
                ...
                if(isset($_POST['category'])) {
                    ...
                    $subcat = isset($_POST['subCategory']) ? $_POST['subCategory'] : false;
                    $header1 = isset($_POST['header1']) ? $_POST['header1'] : false;
                    $intro = isset($_POST['intro']) ? $_POST['intro'] : false;
                    $cbody = isset($_POST['body']) ? $_POST['body'] : false;
                    $timestatus = time();
                    ...
                    $path_to_file_directory = 'categoryimages';
                    $err = "";
                    ...
                    if($filesize > 10000000) {
                        $err = "Image must be smaller than 10 MB";
                        // exit();
                    }
                    $documenturl = 'categoryimages/'. time() . '_' . $filename;
                    
                    $documenturlstrReplace = str_replace(" ", "_", $documenturl);
                    move_uploaded_file($_FILES['image']['tmp_name'], $documenturlstrReplace);
                    
                    $image_files = ['image/png', 'image/gif', 'image/jpg', 'image/jpeg', 'image/webp'];
                    
                    if (in_array($_FILES['image']['type'], $image_files)) {
                        $docType = 'image';
                    }
                    ...
                    if(htmlspecialchars($cat) && htmlspecialchars($cat)) {
                     
                        $con = new PDO("mysql:host=$serverhost;dbname=bitcsvxl_213;" , $serverusername, $serverpassword);
                        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        $invst = $con ->prepare("INSERT INTO fundraisercategories (category,subCategory,intro,header1,body,images,timestatus) VALUES (?,?,?,?,?,?,?)");
                        $invst -> bindParam(1,$cat);
                        $invst -> bindParam(2,$subcat);
                        $invst -> bindParam(3,$intro);
                        $invst -> bindParam(4,$header1);
                        $invst -> bindParam(5,$cbody);
                        $invst -> bindParam(6,$documenturlstrReplace);
                        $invst -> bindParam(7,$timestatus);
                        
                        if($invst -> execute()) {
                        echo $updfe = "<div class='text-center mx-auto bg-white rounded p-1 mt-5 mb-5'>Category successfully added</div>";
                        }
                    }
          ...
          <div class="admin-content">
              <div class="content">
                  ...
                  <h2 class="page-title mx-auto text-center">Add Categories</h2>

                  <form action="" method="post" enctype="multipart/form-data">
                      <input type="hidden" name="id" value="<?php   ?>">
                      <div class="form-group">
                          <label>Category</label>
                          <input type="text" name="category" value="<?php echo @$_POST['category'] ?>" class="form-control">
                      </div>
                      <div class="form-group">
                          <label>Sub category</label>
                          <input type="text" name="subCategory" value="<?php echo @$_POST['subCategory']; ?>" class="form-control">
                      </div>
                      <div class="form-group">
                          <label>Intro</label>
                          <input type="text" name="intro" value="<?php echo @$_POST['intro']; ?>" class="form-control">
                      </div>
                      <div class="form-group">
                          <label>Header1</label>
                          <input type="text" name="header1" value="<?php echo @$_POST['header1']; ?>" class="form-control">
                      </div>
                      <div class="form-group">
                          <label>Body Text</label>
                          <textarea name="body" id="bodytext" class="form-control"><?php echo @$_POST['body']; ?></textarea>
                      </div>
                      <div class="form-group">
                          <label>Image</label>
                      ...
                      <div class="form-group">
                          <button type="submit" name="submit" class="btn btn-big nf">Add Category</button>
                      </div>
                  </form>
